...
module.exports = class PetController {
    static async create(req, res) {
        ...
        const pet = new Pet({
            name, age, weigth, color, available: true,
    ...
    static async getAllUserPets(req, res) {
        ...
        const pets = await Pet.find({ 'user._id': user._id }).select('-user').sort({ 'createdAt': 'desc' })
        res.status(200).json({ pets })
    }
    ...
    static async getAllUserAdoptions(req, res) {
        ...
        const pets = await Pet.find({ 'adopter._id': user._id }).select('-user').sort({ 'createdAt': 'desc' })
        res.status(200).json({ pets })
    }
    ...
    static async edit(req, res) {
        ...
        const updatedData = { name, age, weigth, color, available, images: images.map((image) => image.filename) }
    ...
    static async schedule(req, res) {
        ...
        await pet.save()
        res.status(200).json({ message: `A visita foi agendada com sucesso, entre em contato com ${pet.user.name} através do telefone: ${pet.user.phone}!` })
    }
